<#
Build a distributable Windows package for BlueprintTB.

Usage (from the project root, or anywhere - it cd's itself):
    powershell -ExecutionPolicy Bypass -File scripts\build_windows.ps1

Produces:
    dist\BlueprintTB\                       the built app (onedir)
    dist\BlueprintTB-v<version>-win.zip     zipped app, ready to upload
...
$match = Select-String -Path "blueprinttb\constants.py" -Pattern '^APP_VERSION\s*=\s*"([^"]+)"'
if (-not $match) { throw "Could not find APP_VERSION in blueprinttb\constants.py" }
$version = $match.Matches[0].Groups[1].Value
Write-Host "Building BlueprintTB v$version for Windows..."

if (Test-Path "build") { Remove-Item -Recurse -Force "build" }
if (Test-Path "dist")  { Remove-Item -Recurse -Force "dist" }

python -m PyInstaller blueprinttb.spec --noconfirm
if (-not $?) { throw "PyInstaller build failed - see output above." }

$zipName = "dist\BlueprintTB-v$version-win.zip"
Compress-Archive -Path "dist\BlueprintTB\*" -DestinationPath $zipName -Force

...
from blueprinttb.db.schema import apply_schema
...
def open_workup(path: str | Path, performed_by: str) -> dict:
    """
    Open an existing .btaw file, validate it, log the open event,
...
def _migrate_binder(conn) -> None:
    ...
    if "category" not in tax_line_cols:
        ...
        from blueprinttb.data.tax_line_categories import classify_section, LEGACY_COARSE_CATEGORIES
    ...
    if "prev_hash" not in activity_cols:
        ...
        from blueprinttb.models.activity import compute_row_hash, GENESIS_HASH
        rows = conn.execute(

"""
Pure data-reading helpers for subsidiary .bta.xlsx (or legacy .atbr.xlsx) files — no Qt dependency,
...
def _load_data(xlsx_path: Path) -> dict:
    ...
    if "__data" not in wb.sheetnames:
        raise ValueError("Not a valid Blueprint Trial Balance package")
    ws = wb["__data"]
...
def read_member_info(xlsx_path: Path) -> dict:
    """Read job metadata from a subsidiary .bta.xlsx (or legacy .atbr.xlsx)."""
...
def read_member_financials(
    ...
    """
    Read accounts + entries from subsidiary .bta.xlsx (or legacy .atbr.xlsx).

"""
Rollforward — create a new tax year job from a prior year .bta.xlsx (or legacy .atbr.xlsx) package.
...
def create_rollforward(
    ...
    """
    Create a new .btaw binder rolled forward from *prior_xlsx_path*.
...
def _read_package_data(xlsx_path: Path) -> dict:
    """
    Extract and parse the JSON __data snapshot from a .bta.xlsx (or legacy .atbr.xlsx) file.
    ...
    if "__data" not in wb.sheetnames:
        raise ValueError("This file does not contain a __data sheet. "
                         "Please select a valid Blueprint Trial Balance review package.")

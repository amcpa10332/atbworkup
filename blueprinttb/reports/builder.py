...
class ReportLine:
    ...
    line_name:      str   = ""  # tax line (e.g. "Cash") -- one tier finer than
                                 # ReportSection.name, which is the tax line's
                                 # parent bucket (e.g. "Current Assets")
...
@dataclass
class ReportSection:
    """One tax-line BUCKET (e.g. "Current Assets"), matching tax_lines.section
    in the DB and the exporter's Excel Tax Grouping tab -- one level coarser
    than an individual tax line (e.g. "Cash"), which lives on each
    ReportLine.line_name. A bucket typically holds several tax lines."""
    name:       str
    sort_order: int
...
def build_report(conn, job_id: str, statement: str) -> FinancialReport:
    ...
    for row in rows:
        ...
        sections[sec_name].lines.append(ReportLine(
            ...
            line_name      = row["line_name"] or "Unmapped",

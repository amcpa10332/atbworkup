"""
Review package exporter — produces a `.bta.xlsx` file (formerly `.atbr.xlsx`
when this app was named ATBWorkup -- see utils/naming.py:PACKAGE_EXTENSIONS
for the backward-compat list every "Open" dialog and importer accepts).
...
def suggested_filename(job: dict, version: int | None = None) -> str:
    from blueprinttb.utils.naming import suggested_filename as _sfn
...
def transition_workup_status(
    ...
    *,
    ...
    .bta.xlsx in output_dir with the new name, and return (updated_job, new_path).
    """
    from blueprinttb.models.job import transition_status, get_job
    from blueprinttb.utils.naming import suggested_filename as _sfn
    from blueprinttb.db.connection import db_connection as _dbc
...
def export_review_package(conn, *, job: dict, output_path: str | Path,
    ...
    """Build and write the .bta.xlsx file. Returns the inserted packages row."""
    output_path = Path(output_path)
...
def _build_workbook(conn, job: dict, version: int, performed_by: str,
    ...
    """
    Construct the full .bta.xlsx openpyxl Workbook in memory.
...
# subsidiary's own .bta.xlsx. Read and combine them the same way the
...
def _write_cover_tab(wb, job: dict, version: int, performed_by: str, now_str: str):
    ...
    pkg_fields = [
        ...
        ("File Format",      ".bta.xlsx (Blueprint Trial Balance Review Package)"),
    ]
...
def _write_data_tab(wb, conn, job: dict) -> str:
    ...
    # the .bta.xlsx is what students actually submit, so the audit trail has
    # to be verifiable from that file alone, not just the original .btaw.
    activity_log = [
        dict(r) for r in conn.execute(
    ...
    tax_grouping_ties = [
        dict(r) for r in conn.execute(
            "SELECT * FROM tax_grouping_ties WHERE job_id = ?",
            (job["job_id"],),
        )
    ]
    ...
    payload = {
        ...
        "tax_grouping_ties":     tax_grouping_ties,
    }
...
def _write_manifest_tab(wb, conn, job: dict, version: int, performed_by: str,
    ...
    from blueprinttb.models.activity import verify_activity_chain

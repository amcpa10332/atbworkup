...
def _migrate_settings_schema(conn) -> bool:
    ...
    if "category" not in tlt_cols:
        ...
        from blueprinttb.data.tax_line_categories import LEGACY_COARSE_CATEGORIES
    ...
    if not reseed:
        # DEFAULT_TAX_LINES is a static built-in catalog with no user data
        # (see the category-migration comment above), so adding new lines
        # to it -- e.g. a new tax line for an entity type -- is meant to be
        # picked up by a reseed same as a schema change. Without this check,
        # ensure_settings_db() only reseeds on column changes: `count == 0
        # or reseed` at the call site would see a non-empty, structurally
        # unchanged table and skip re-seeding forever, silently hiding any
        # newly added default line from every existing installation.
        from blueprinttb.data.tax_line_seeds import DEFAULT_TAX_LINES
        expected_codes = {(row[0], row[4]) for row in DEFAULT_TAX_LINES}
        existing_codes = {
            (row["entity_type"], row["line_code"])
            for row in conn.execute("SELECT entity_type, line_code FROM tax_line_templates")
        }
...
def _seed_default_templates(conn) -> None:
    import uuid
    from blueprinttb.data.tax_line_seeds import DEFAULT_TAX_LINES, TEMPLATE_DISPLAY_NAMES
    from blueprinttb.data.tax_line_categories import classify_section
    conn.executemany(

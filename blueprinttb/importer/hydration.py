...
into an already-created .btaw SQLite binder (the job row already exists).
...
def hydrate_binder(conn, data: dict) -> None:
    ...
    # FKs to accounts, so must come after the accounts loop above.
    for t in data.get("tax_grouping_ties", []):
        conn.execute(
            """
            INSERT OR IGNORE INTO tax_grouping_ties
              (account_id, job_id, flag, tied_value, tied_by, tied_at)
            VALUES (?,?,?,?,?,?)
            """,
            (
                t["account_id"], t["job_id"],
                t.get("flag"),
                t.get("tied_value"),
                t.get("tied_by"),
                t.get("tied_at") or now,
            ),
        )

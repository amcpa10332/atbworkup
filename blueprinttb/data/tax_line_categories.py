...
Categories:
    ...
    schedule_k           — pass-through items (1065/1120S); mixes income-
                           ...
                           section sum
    nol_deduction        — C-corp (1120) Schedule M-1: Net Operating Loss
                           deduction, its own bucket/subtotal below
                           Deductions, ahead of Net Income
    drd_deduction        — C-corp (1120) Schedule M-1: Dividends Received
                           Deduction, its own bucket/subtotal, positioned
                           after the NOL deduction since DRD is computed
                           on income after the NOL is absorbed
...
CATEGORY_NOL          = "nol_deduction"
CATEGORY_DRD          = "drd_deduction"
...
def classify_section(financial_statement: str, section: str) -> str:
    ...
    if "net operating loss" in s or s == "nol" or "nol deduction" in s:
        return CATEGORY_NOL
    if "dividends received" in s or s == "drd" or "drd deduction" in s:
        return CATEGORY_DRD

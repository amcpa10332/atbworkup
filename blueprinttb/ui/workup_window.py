...
from blueprinttb.constants import ENTITY_TYPES, WORKPAPER_ENTITY_TYPES
from blueprinttb.utils.naming import STATUS_COLORS
...
class WorkupWindow(QMainWindow):
    ...
    _SAVE_BTN_STYLE_NORMAL = (
        "background:#4A7A4A;color:#FFFFFF;font-weight:bold;"
        "padding:5px 14px;border-radius:3px;"
    )
    _SAVE_BTN_STYLE_JUST_SAVED = (
        "background:#D08A1A;color:#FFFFFF;font-weight:bold;"
        "padding:5px 14px;border-radius:3px;"
    )
    ...
    def _build_ui(self):
        ...
        # status bar
        sb = QStatusBar()
        from blueprinttb.constants import ROLE_COLORS
    ...
    def _build_toolbar(self):
        ...
        save_col = QWidget()
        save_col_layout = QVBoxLayout(save_col)
        save_col_layout.setContentsMargins(0, 0, 0, 0)
        save_col_layout.setSpacing(0)

        self._save_btn = QPushButton("Save")
        self._save_btn.setStyleSheet(self._SAVE_BTN_STYLE_NORMAL)
        self._save_btn.setEnabled(self._source_xlsx is not None)
        self._save_btn.clicked.connect(self._on_save)
        save_col_layout.addWidget(self._save_btn)

        self._last_saved_label = QLabel("Not saved yet this session")
        self._last_saved_label.setStyleSheet(
            "color:#666666; font-size:10px; padding:1px 2px;"
        )
        save_col_layout.addWidget(self._last_saved_label)

        tb.addWidget(save_col)

        self._save_as_btn = QPushButton("Save As…")
        self._save_as_btn.setToolTip(
            "Emergency backup: save a copy of everything in this job to a "
            "file you choose, without touching the original file."
        )
        self._save_as_btn.setStyleSheet(
            "background:#5A6A8A;color:#FFFFFF;font-weight:bold;"
            "padding:5px 14px;border-radius:3px;"
        )
        self._save_as_btn.clicked.connect(self._on_save_as)
        tb.addWidget(self._save_as_btn)
    ...
    def _rebuild_fs_tab(self):
        ...
        if count == 0:
            ...
            from blueprinttb.ui.financial_grid import FinancialGrid
            ...
            self._grid.data_changed.connect(self._reprint_report)
            self._grid.data_changed.connect(self._refresh_audit_log)
    ...
    def _tab_index_by_text(self, text: str) -> int:
        for i in range(self._tabs.count()):
            if self._tabs.tabText(i) == text:
                return i
        return -1
    ...
    def _rebuild_report_tab(self):
        ...
        # Remove the PREVIOUS "Reports" tab by its label, not by a hardcoded
        # index. Other tabs (notably Audit Log) can end up sitting at that
        # index before Reports has ever been created — e.g. on first launch,
        # before any TB is imported, Reports doesn't exist yet, so Audit Log
        # occupies its slot. Removing "whatever is at index 2" then silently
        # deleted Audit Log instead of a nonexistent Reports tab.
        existing = self._tab_index_by_text("Reports")
        if existing != -1:
            self._tabs.removeTab(existing)
        self._report_tab = None

        if count == 0:
            return

        from blueprinttb.ui.report_tab import ReportTab
        # PDF export should default to the user's actual workpaper folder,
        # not self._path (the app's ephemeral temp working file) -- see
        # ReportTab.__init__'s default_export_dir docstring.
        export_dir = (
            self._source_xlsx.parent if self._source_xlsx
            else self._job.get("workpaper_folder") or None
        )
        self._report_tab = ReportTab(
            self._path, self._job["job_id"], parent=self, default_export_dir=export_dir,
            performed_by=self._performed_by,
        )
        ...
        self._tabs.insertTab(self._fs_tab_index + 1, self._report_tab, "Reports")
    ...
    def _rebuild_audit_tab(self):
        existing = self._tab_index_by_text("Audit Log")
        if existing != -1:
            self._tabs.removeTab(existing)
        self._audit_tab = None

        from blueprinttb.ui.audit_log_tab import AuditLogTab
        self._audit_tab = AuditLogTab(self._path, self._job["job_id"], parent=self)
        # Slot in right after Reports if it exists yet, else right after
        # Trial Balance — never a hardcoded index, since Reports may not
        # have been created yet (no TB imported) when this first runs.
        reports_idx = self._tab_index_by_text("Reports")
        insert_at = reports_idx + 1 if reports_idx != -1 else self._fs_tab_index + 1
        self._tabs.insertTab(insert_at, self._audit_tab, "Audit Log")
    ...
    def _rebuild_workpapers_tab(self):
        ...
        from blueprinttb.ui.workpapers_tab import WorkpapersTab
    ...
    def _on_tab_changed(self, index: int):
        # Compare by widget identity, not a fixed index -- Reports/Audit
        # Log/Workpapers tabs shift position depending on which ones exist
        # yet (e.g. before a TB is imported), so a hardcoded index goes
        # stale the moment tabs are inserted/removed around it.
        current_widget = self._tabs.widget(index)
        if current_widget is self._audit_tab and self._audit_tab is not None:
            self._audit_tab.refresh()
        if current_widget is self._workpapers_tab and self._workpapers_tab is not None:
    ...
    def _rebuild_je_panel(self):
        ...
        from blueprinttb.ui.je_panel import JEPanel
    ...
    def _rebuild_notes_dock(self):
        ...
        from blueprinttb.ui.notes_panel import NotesDock
        ...
        self._notes_dock.notes_changed.connect(self._refresh_audit_log)
    ...
    def _refresh_notes_badge(self):
        from blueprinttb.db.connection import db_connection
        from blueprinttb.models.notes import open_note_count
        with db_connection(self._path) as conn:
    ...
    def _refresh_reviewer_grid_flags(self):
        if not self._grid:
            return
        from blueprinttb.db.connection import db_connection
        from blueprinttb.models.notes import reviewer_note_account_ids
        with db_connection(self._path) as conn:
    ...
    def _add_note(self, *, context_label: str, linked_to_type: str, linked_to_id: str):
        from blueprinttb.ui.add_note_dialog import AddNoteDialog
        from blueprinttb.db.connection import db_connection
        from blueprinttb.models.notes import create_note
        from blueprinttb.models.activity import log_activity
    ...
    def _flag_just_saved(self):
        """Briefly turn the Save button orange and stamp the last-saved time,
        so a manual save gives visible confirmation instead of only a
        status-bar message that's easy to miss."""
        from PySide6.QtCore import QTimer
        stamp = time.strftime("%I:%M:%S %p").lstrip("0")
        self._last_saved_label.setText(f"Last saved {stamp}")
        self._save_btn.setStyleSheet(self._SAVE_BTN_STYLE_JUST_SAVED)
        QTimer.singleShot(
            2500,
            lambda: self._save_btn.setStyleSheet(self._SAVE_BTN_STYLE_NORMAL),
        )

    def _on_save_as(self):
        """Emergency backup: export everything in this job's working database
        to a new .bta.xlsx of the user's choosing, independent of the file
        this window normally saves to. Does not change what Save writes to —
        purely a safety-net copy for situations like a broken working file."""
        from blueprinttb.utils.naming import with_package_extension
        default_name = self._source_xlsx.name if self._source_xlsx else "workup.bta.xlsx"
        chosen, _ = QFileDialog.getSaveFileName(
            self, "Save a Copy As", default_name,
            "Blueprint Trial Balance Files (*.bta.xlsx *.atbr.xlsx)",
        )
        if not chosen:
            return
        chosen = with_package_extension(chosen)
        from blueprinttb.db.connection import db_connection
        from blueprinttb.exporter.review_package import save_workup
        try:
            with db_connection(self._path) as conn:
            ...
            QMessageBox.information(
                self, "Saved a Copy",
                f"A full copy of this job was saved to:\n\n{chosen}",
            )
        except Exception as exc:
            QMessageBox.critical(self, "Save As Error", f"Failed to save a copy:\n{exc}")
    ...
    def _execute_transition(self, new_status: str, lock: bool = False):
        from blueprinttb.db.connection import db_connection
        from blueprinttb.exporter.review_package import transition_workup_status
    ...
    def _do_fresh_import_tb(self):
        from blueprinttb.ui.tb_import_wizard import TBImportWizard
        ...
        mapped_msg = f"\n{stats.auto_mapped} account(s) auto-mapped from pre-mapped codes." if stats.auto_mapped else ""
        unrecognized_msg = (
            "\nUnrecognized codes (mapped manually instead): " + ", ".join(stats.unrecognized_codes)
        ) if stats.unrecognized_codes else ""
        QMessageBox.information(
            self, "Import Complete",
            f"{stats.inserted} accounts imported{bal_msg}.\n\n"
            f"Total debits:  {result.total_debits:,.2f}\n"
            f"Total credits: {result.total_credits:,.2f}"
            f"{mapped_msg}{unrecognized_msg}",
        )
        if stats.auto_mapped < stats.inserted:
            self._open_mapping_workbench()

    def _do_reimport_tb(self):
        from blueprinttb.ui.tb_import_wizard import TBImportWizard
        from blueprinttb.importer.tb_writer import reimport_accounts
        from blueprinttb.db.connection import db_connection
    ...
    def _on_edit_account_by_id(self, account_id: str):
        from blueprinttb.db.connection import db_connection
        from blueprinttb.models.accounts import update_account
    ...
    def _open_mapping_workbench(self):
        from blueprinttb.ui.mapping_workbench import MappingWorkbench

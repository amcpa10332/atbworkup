...
from PySide6.QtCore import Qt, Signal, QStringListModel, QSortFilterProxyModel, QSize
from PySide6.QtWidgets import QMenu
from PySide6.QtGui import QColor, QFont, QKeyEvent, QIcon, QPixmap, QPainter, QPen
...
_EDITOR_COLS = ["Account", "DR", "CR", "Memo", "Del"]
...
class JEPanel(QWidget):
    ...
    def __init__(self, path: str | Path, job_id: str, performed_by: str,
        ...
        # Dirty-tracking: deleting/editing a line only mutates this table in
        # memory -- nothing is persisted until Save is clicked. Without this,
        # switching to a different entry (or starting a new one) silently
        # reloaded the ORIGINAL lines from the DB with no warning, which is
        # exactly what "deleting a line doesn't work" looks like from the
        # preparer's side: the delete visibly happens, then reverts the
        # moment they move on without saving.
        self._dirty  = False
        self._loading = False
    ...
    def _build_ui(self):
        ...
        self._lines_table.setColumnWidth(_COL_DEL, 36)
        self._lines_table.horizontalHeaderItem(_COL_DEL).setToolTip(
            "Click the trash icon on a line to delete it, or right-click any\n"
            "field in the line and choose \"Delete Line\"."
        )
    ...
    def _on_list_select(self, row: int, *_):
        ...
        if not item:
            return
        target_aje_id = item.data(Qt.UserRole)
        if target_aje_id == self._current_aje_id:
            return
        if not self._confirm_discard_if_dirty():
            ...
            self._list.blockSignals(False)
            return
        self._load_entry(target_aje_id)
    ...
    def _confirm_discard_if_dirty(self) -> bool:
        """Return True if it's fine to proceed (nothing unsaved, or the user
        explicitly chose to save/discard); False means stay put (Cancel)."""
        if not self._dirty:
            return True
        reply = QMessageBox.question(
            self, "Unsaved Changes",
            "This journal entry has unsaved changes (e.g. a deleted or "
    ...
    def _append_line_row(self, account_id: str | None = None,
        ...
        memo_edit.textChanged.connect(self._mark_dirty)
        ...
        del_btn = QPushButton()
        del_btn.setIcon(_trash_icon())
        del_btn.setIconSize(QSize(15, 15))
        del_btn.setFixedSize(24, 24)
        del_btn.setFocusPolicy(Qt.NoFocus)
        del_btn.setToolTip("Delete this line")
        del_btn.setStyleSheet(
            "QPushButton { background: transparent; border: none; }"
            "QPushButton:hover { background: #F3D6D6; border-radius: 3px; }"
        )
        del_btn.clicked.connect(lambda _=False, row=r: self._remove_line_row(row))
        self._lines_table.setCellWidget(r, _COL_DEL, del_btn)

        # Right-click on ANY field in the line also offers "Delete Line" --
        # right-clicking a text field previously only showed the OS's plain
        # Cut/Copy/Paste/Delete menu (whose "Delete" just deletes selected
        # text), which preparers expected to delete the whole line.
        # The account combo needs its own entry here in addition to its
        # lineEdit(): the combo's dropdown-arrow strip is part of the combo
        # box itself, not the line edit, so a right-click landing on that
        # strip previously reached neither handler and silently did nothing.
        for w in (combo, combo.lineEdit(), dr, cr, memo_edit):
            w.setContextMenuPolicy(Qt.CustomContextMenu)
            w.customContextMenuRequested.connect(
                lambda pos, w=w: self._show_line_context_menu(w, pos)
            )
    ...
    def _row_of_widget(self, widget) -> int:
        """Find widget's current row by scanning cell widgets, rather than
        capturing a row number at creation time -- rows shift whenever an
        earlier line is deleted, and this is only looked up at click time."""
        for r in range(self._lines_table.rowCount()):
            for c in range(self._lines_table.columnCount()):
                cw = self._lines_table.cellWidget(r, c)
                if cw is widget or (isinstance(cw, _AccountCombo) and cw.lineEdit() is widget):
                    return r
        return -1

    def _show_line_context_menu(self, widget, pos):
        # QComboBox has no createStandardContextMenu() (that's a QLineEdit-only
        # method) -- it's only reachable here for a click on the combo's own
        # dropdown-arrow strip, which has no cut/copy/paste of its own anyway.
        if hasattr(widget, "createStandardContextMenu"):
            menu = widget.createStandardContextMenu()
            menu.addSeparator()
        else:
            menu = QMenu(widget)
        del_action = menu.addAction("Delete Line")
        action = menu.exec(widget.mapToGlobal(pos))
        if action == del_action:
            row = self._row_of_widget(widget)
            if row >= 0:
                self._remove_line_row(row)
    ...
    def _update_balance(self, *_):
        ...
        # Save must stay available even when out of balance -- an entry can be
        # intentionally saved as an unbalanced "Shell" (see save_lines /
        # _refresh_balance_flag in models/journal_entries.py). Gating Save on
        # `balanced` meant deleting a line from a balanced entry (which almost
        # always leaves the remainder unbalanced) silently disabled Save, so
        # the deletion was never persisted and reappeared on next reload.
        self._save_btn.setEnabled(bool(lines))
    ...
    def _do_signoff(self):
        ...
        with db_connection(self._path) as conn:
            ...
            if entry and entry.get("reviewer_signoff_by"):
                ...
                event, desc = "removed_signoff", f"Removed reviewer sign-off: {entry.get('description', '')}"
            else:
                signoff_entry(conn, self._current_aje_id, self._performed_by)
                event, desc = "signoff_aje", f"Reviewer sign-off: {entry.get('description', '')}"
            log_activity(
                conn,
                job_id=self._job_id,
                event_type=event,
...
def _trash_icon(color: str = "#C62828", size: int = 16) -> QIcon:
    """Draw a small monochrome trash-can glyph so the delete affordance reads
    as a delete action at a glance, instead of relying on a bare '×'."""
    ...
    left, right, top, bottom = size * 0.20, size * 0.80, size * 0.30, size * 0.88
    ...
    # Lid
    painter.drawLine(int(size * 0.12), int(top), int(size * 0.88), int(top))
    # Handle
    painter.drawLine(int(size * 0.38), int(top), int(size * 0.38), int(size * 0.16))
    painter.drawLine(int(size * 0.62), int(top), int(size * 0.62), int(size * 0.16))
    painter.drawLine(int(size * 0.38), int(size * 0.16), int(size * 0.62), int(size * 0.16))
    # Shred lines inside the bin
    for frac in (0.38, 0.5, 0.62):
        painter.drawLine(int(size * frac), int(top + size * 0.10),
                          int(size * frac), int(bottom - size * 0.08))
    painter.end()
    return QIcon(pixmap)

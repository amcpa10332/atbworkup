...
class TBImportWizard(QWizard):
    def __init__(self, parent=None, initial_dir: str = "", entity_type: str = ""):
...
class _FileSheetPage(QWizardPage):
    ...
    def _build_ui(self):
        ...
        # ── Pre-mapped import ────────────────────────────────────────────
        pm_group = QGroupBox("Pre-Mapped Import")
        pm_layout = QVBoxLayout(pm_group)
        self._pre_mapped_check = QCheckBox(
            "I would like to import a pre-mapped trial balance"
        )
        self._pre_mapped_check.toggled.connect(self._on_pre_mapped_toggled)
        pm_layout.addWidget(self._pre_mapped_check)

        pm_row = QHBoxLayout()
        self._pm_hint = QLabel(
            "Download the list of valid tax line codes for this entity type, add a\n"
            "matching code to each row of your trial balance, then upload it above."
        )
        self._pm_hint.setStyleSheet("color: #555555; font-size: 11px;")
        self._pm_download_btn = QPushButton("Download Tax Line Codes…")
        self._pm_download_btn.clicked.connect(self._download_template)
        pm_row.addWidget(self._pm_hint, 1)
        pm_row.addWidget(self._pm_download_btn)
        pm_layout.addLayout(pm_row)
        self._pm_hint.setVisible(False)
        self._pm_download_btn.setVisible(False)
    ...
    def _on_pre_mapped_toggled(self, checked: bool):
        self._pm_hint.setVisible(checked)
        self._pm_download_btn.setVisible(checked)

    def _download_template(self):
        entity_type = self._wiz._entity_type
        if not entity_type:
            QMessageBox.warning(self, "Unavailable",
                                "This job has no entity type set yet.")
            return
        from blueprinttb.data.tax_line_seeds import TEMPLATE_DISPLAY_NAMES
        display = TEMPLATE_DISPLAY_NAMES.get(entity_type, entity_type)
        path, _ = QFileDialog.getSaveFileName(
            self, "Save Tax Line Codes",
            f"{entity_type} Tax Line Codes.xlsx",
            "Excel Files (*.xlsx)",
        )
        if not path:
            return
        try:
            from blueprinttb.exporter.template_exporter import export_template_to_excel
            export_template_to_excel(entity_type, path)
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Could not write file:\n{e}")
            return
        QMessageBox.information(
            self, "Downloaded",
            f"Valid tax line codes for {display} saved to:\n{path}\n\n"
            "Add a column to your trial balance with the matching \"line_code\" "
            "value for each account, then upload it here.",
        )

    @property
    def is_pre_mapped(self) -> bool:
        return self._pre_mapped_check.isChecked()
...
class _ColumnSetupPage(QWizardPage):
    ...
    def _build_ui(self):
        ...
        self._grouping_label = QLabel("Tax line code column:")
        self._grouping_combo = QComboBox()
        self._grouping_combo.setMinimumWidth(220)
        grid.addWidget(self._grouping_label, 4, 0, Qt.AlignRight)
        grid.addWidget(self._grouping_combo, 4, 1)
        self._grouping_label.setVisible(False)
        self._grouping_combo.setVisible(False)
    ...
    def initializePage(self):
        ...
        pre_mapped = self._wiz._page_file.is_pre_mapped
        self._grouping_label.setVisible(pre_mapped)
        self._grouping_combo.setVisible(pre_mapped)
    ...
    def _rebuild_combos(self, rows, header_row_0based: int):
        ...
        prev = {
            ...
            self._grouping_combo: self._grouping_combo.currentIndex(),
        }
        for combo in prev:
    ...
    def _build_config(self) -> dict:
        ...
        if self._wiz._page_file.is_pre_mapped:
            cfg["grouping_col"] = idx(self._grouping_combo)
        if self._mode_single.isChecked():
...
class _ConfirmPage(QWizardPage):
    ...
    def initializePage(self):
        ...
        pm_html = ""
        if self._wiz._page_file.is_pre_mapped:
            with_code = sum(1 for a in r.accounts if a.tax_line_code)
            pm_html = (
                f"<br><b>{with_code}</b> of these rows include a tax line code and "
                "will be auto-mapped where the code is recognized; the rest land in "
                "the manual mapping window as usual.<br>"
            )
...
def _guess_balance_col(
    ...
    from blueprinttb.importer.tb_parser import _try_parse_amount
    for c in range(n_cols):
        if c == skip_col:
            continue
        numeric = sum(

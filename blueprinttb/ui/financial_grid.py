...
_FLAG_DISPLAY = {
    ...
    "issue":    ("✗", QColor("#c62828")),
    # No glyph for the unset state -- the light gray fill (_FLAG_BOX_BG)
    # plus the drawn outline (see _FlagBoxDelegate) are what mark this as a
    # clickable box; an actual "▢" character on top of that read as a
    # separate, ugly little icon rather than one clean box.
    None:       ("", QColor("#6B7280")),
}
_FLAG_BOX_BG = QColor("#EBEDF2")   # light gray fill behind the flag cell, kept as-is
_FLAG_BOX_OUTLINE = QColor(0, 0, 0, 90)   # faint (~35% opacity) black outline around that fill
...
class _FlagBoxDelegate(RoleBackgroundDelegate):
    """Same background-role fix as RoleBackgroundDelegate, plus a faint
    outline drawn around the flag column specifically -- replaces the old
    "▢" glyph, which read as an ugly little icon rather than a clean
    clickable box. The light gray fill (_FLAG_BOX_BG) stays exactly as
    before; this just adds a thin border around it."""

    def paint(self, painter, option, index):
        super().paint(painter, option, index)
        if index.column() != COL_FLAG:
            return
        # Only account rows get the flag-box fill (_FLAG_BOX_BG) -- header/
        # group rows' flag cell is background-filled with THEIR row color
        # instead, so checking for that specific color (rather than "any
        # background is set") keeps the outline off non-clickable rows.
        bg = index.data(Qt.BackgroundRole)
        if bg is not None and bg.color() == _FLAG_BOX_BG:
            painter.save()
            painter.setPen(QPen(_FLAG_BOX_OUTLINE, 1))
...
class FinancialGrid(QWidget):
    ...
    data_changed      = Signal()           # emitted after grouping/mapping/rename/new-account
                                            # -- anything the Reports tab and audit log should
                                            # reflect immediately rather than on next tab switch
    ...
    def _build_ui(self):
        ...
        # False, not True: Qt's native expand/collapse arrow is painted in a
        # reserved strip that ignores the row's own custom background/
        # foreground (a Qt limitation, same family as the one fixed by
        # RoleBackgroundDelegate) -- it always painted plain white there,
        # jarring next to an orange/gray header row. Disabling it lets that
        # strip inherit the row's real background (verified by rendering
        # and reading back actual pixel colors); _make_section_row adds a
        # manual "▾" prefix so a row still visually reads as expandable,
        # matching the "▸" _make_group_row already prefixes group rows
        # with. Clicking anywhere on a header/group row still toggles
        # expand/collapse (see _on_item_clicked), so nothing here was ever
        # reachable only via the arrow itself.
        ...
        self._tree.setItemDelegate(_FlagBoxDelegate(self._tree))
    ...
    def refresh(self):
        collapsed = self._capture_collapsed()
        self._tree.clear()
    ...
    # refresh() is called after nearly every action (grouping, mapping,
    # rename, new account, JE save, note add) -- without this, each of
    # those would blow away whatever the user had collapsed via the
    # unconditional expandAll() above. Keys are built from label text
    # (with the group's "(N accounts)" count stripped, since that count
    # can change without the user's collapse choice changing) rather than
    # ids, since account rows themselves carry no stable position to key on.

    _GROUP_LABEL_RE = re.compile(r"^▸ (.*?)  \(\d+ accounts?\)$")

    def _expand_key_part(self, item: QTreeWidgetItem) -> str:
        text = item.text(COL_NAME)
        m = self._GROUP_LABEL_RE.match(text)
        return m.group(1) if m else text

    def _capture_collapsed(self) -> set[str]:
        collapsed: set[str] = set()

        def walk(item: QTreeWidgetItem, path: str):
            key = f"{path}/{self._expand_key_part(item)}"
            if item.childCount() and not item.isExpanded():
                collapsed.add(key)
            for i in range(item.childCount()):
                walk(item.child(i), key)
    ...
    def _make_section_row(
        ...
        # "▾" stands in for the native expand arrow (now disabled -- see
        # setRootIsDecorated(False) in _build_ui) so the row still reads as
        # expandable, same convention _make_group_row already uses ("▸").
        item.setText(COL_NAME, f"▾  {label.strip()}")
    ...
    def _make_group_row(self, name: str, accounts: list[dict]) -> QTreeWidgetItem:
        ...
        item.setFont(COL_NAME, font)
        # Same platinum/black tier as the tax-line subsection header above
        # it, per firm request -- tax-line and group rows are meant to read
        # as the same "grouping" tier, distinguished by indentation rather
        # than by a different color.
        bg = SUBSECTION_BG
        fg = SUBSECTION_FG
        for c in range(len(HEADERS)):
            item.setBackground(c, QBrush(bg))
    ...
    def _make_account_row(self, acct: dict, alt: bool, in_group: bool = False) -> QTreeWidgetItem:
        ...
        if in_group:
            # Faint, flat navy tint ties these rows visually to their group
            # header -- no alternation, matching plain account rows below.
            bg = QColor("#EEF1F9")
        else:
            bg = ROW_BG_ODD
        for c in range(len(HEADERS)):
        ...
        # Flag cell gets its own tinted "box" background, independent of the
        # row's alternating shade, so the clickable area is visually
        # distinct instead of blending into whatever the row happens to be.
        item.setBackground(COL_FLAG, QBrush(_FLAG_BOX_BG))
    ...
    def _cycle_flag(self, item: QTreeWidgetItem, account_id: str):
        ...
        with db_connection(self._path) as conn:
            ...
            log_activity(
                ...
                description=f"Flag set to '{next_flag or 'none'}': {item.text(COL_NAME).strip()}",
                performed_by=self._performed_by,
            )
        # Every other mutating handler in this file (mapping, grouping,
        # delete, new/edit account) emits data_changed so the Reports tab
        # and Audit Log tab stay current -- flag-cycling was the one
        # exception, so tick-marking an account never refreshed either.
    ...
    def _open_mapping_dialog(self, account_ids: list[str]):
        from blueprinttb.ui.mapping_dialog import MappingDialog
    ...
    def _open_group_dialog(self, account_ids: list[str]):
        from blueprinttb.ui.group_dialog import GroupPickerDialog
        from blueprinttb.models.groups import add_accounts_to_group
        with db_connection(self._path) as conn:
            names = []
            for aid in account_ids:
                row = conn.execute(
    ...
    def _on_new_account(self):
        from blueprinttb.ui.new_account_dialog import NewAccountDialog

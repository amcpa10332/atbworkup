...
from blueprinttb.models.notes import (
    get_notes, clear_note, resolve_note, open_note_count, create_note,
    update_note,
)
from blueprinttb.constants import NOTE_TYPE_COLORS
...
class NotesDock(QDockWidget):
    ...
    def reload(self):
        ...
        if not notes:
            ...
            for note in notes:
                ...
                card.reply_requested.connect(self._on_reply_note)
    ...
    def _on_reply_note(self, note_id: str, current_body: str):
        """Append a timestamped reply to the note's body, keeping it open --
        lets a preparer answer a reviewer note (or vice versa) in place
        instead of only being able to Edit (replace) or Clear/Resolve (close)
        it."""
        dlg = _ReplyNoteDialog(current_body, parent=self)
        if dlg.exec() != _ReplyNoteDialog.Accepted:
            return
        reply_text = dlg.reply_text()
        if not reply_text:
            return
        import datetime as _dt
        stamp = _dt.datetime.now().strftime("%m/%d/%y %I:%M %p").lstrip("0")
        new_body = (
            f"{current_body}\n\n— Reply by {self._performed_by} ({stamp}):\n{reply_text}"
        )
        with db_connection(self._path) as conn:
            update_note(conn, note_id, body=new_body)
        self.reload()
...
class _NoteCard(QFrame):
    ...
    reply_requested    = Signal(str, str)   # note_id, current body
    ...
    def __init__(self, note: dict, performed_by: str, role: str = "preparer",
        ...
        if is_open:
            ...
            reply_btn.clicked.connect(
                lambda: self.reply_requested.emit(note["note_id"], note["body"])
            )
            ...
            edit_btn.clicked.connect(
                lambda: self.edit_requested.emit(note["note_id"], note["body"])
            )
...
class _ReplyNoteDialog(QDialog):
    """Add a reply appended under the existing note body -- keeps the note
    open (unlike Clear/Resolve) and preserves the original text (unlike
    Edit, which replaces it)."""

    def __init__(self, current_body: str, parent=None):
        super().__init__(parent)
        ...
        layout.addWidget(QLabel("Original note:"))
        original = QLabel(current_body)
        original.setWordWrap(True)
        original.setStyleSheet(
            "color: #555555; background: #F0F0F0; border-radius: 3px; padding: 6px;"
        )
        layout.addWidget(original)

        layout.addWidget(QLabel("Your reply:"))
        self._reply_edit = QTextEdit()
        self._reply_edit.setMinimumHeight(80)
        layout.addWidget(self._reply_edit)

        buttons = QDialogButtonBox(
            QDialogButtonBox.Save | QDialogButtonBox.Cancel
        )
        buttons.button(QDialogButtonBox.Save).setText("Post Reply")
        buttons.accepted.connect(self.accept)
        buttons.rejected.connect(self.reject)
        layout.addWidget(buttons)

        self._reply_edit.setFocus()

    def reply_text(self) -> str:
        return self._reply_edit.toPlainText().strip()
